...
| サウンドノベル書き出し（ゲーム化・演出譜）を変える | `src/core/game/`（判別・演出譜・テンプレ背景・持ち込み背景・`stage.ts`＝立ち絵の席（左・中央・右）を行ごとに解く `resolveStages`＝書き出しと画面が共用・`continuity.ts`＝「この行で何が効いているか」の解決）+ `src/core/exporter/toNovelGame.ts`（プレイヤー本体は `novelGamePlayer.ts`。**場面の切れ目＝暗転＋間**（D-GAME-SCENE-CURTAIN）は同ファイルの `changeScene`＝幕 `#curtain` を下ろして BGM・環境音を絞り、幕の下で背景・立ち絵を組んでフェードで明ける。時間は `CURTAIN_OUT/HOLD/IN`）。演出エディタは `src/ui/components/StagingView/`（欄の説明は `field-helps.tsx`、アプリ内プレビューは `preview-dialog.tsx`＝`buildNovelGameHtml` を sandbox iframe に流し込む・`startAt` でその行から）、永続化は `src/core/storage/stagingRepository.ts`（演出譜）と `gameAssetRepository.ts`（持ち込み背景） |
| サウンドノベルの効果音（合成SE＋運営テンプレの音声ファイル）を変える／**出す・しまう** | **いまは画面・MCP・書き出しから外している**（D-GAME-SE-HOLD）。出し入れは `src/core/game/features.ts` の `GAME_FEATURES.se` だけ（演出エディタの欄とレーン・`resolveTemplateSes`・exporter の `se` 読み飛ばし・MCP のスキーマと `setStagingCues`・管理ページのタブが揃って切り替わる。フラグを立てた状態の検証は各 `*.test` が `vi.mock('@/core/game/features')` で、落ちた状態は `*.features.test`）。合成レシピは `src/core/game/sePresets.ts`（`PRESET_SES`＝12 種・`presetSe` `sePeriod`＝環境音のループ周期。レシピの語彙は白/ピンク/ブラウンのノイズ・hp/bp/lp・a/s のエンベロープ・mf/md の揺らぎ・rv の残響。鳴らし方は `Cue.seRepeat`＝1回/2回/ループ・止めるのは予約キー `SE_STOP`）。音声ファイル（目録 kind `se`・mp3/m4a）は `templates.ts` の `mergeSeCatalog` で合成に重なり、exporter がシナリオ `ses[key]` を `{steps}`（合成）か `{src}`（ファイル・zip は `assets/se/<slug>.mp3`・投稿は契約 v6 の `asset:<id>`）で載せる。解釈はプレイヤー（`novelGamePlayer.ts` の `playSe` `startLoopSe`＝`src` は fetch→decodeAudioData）とアプリ試聴 `src/ui/_utils/sePlayer.ts`（`playCatalogSe`）の2箇所——**両方を同時に直す** |
| サウンドノベルの **BGM**（運営のオリジナル曲・D-GAME-BGM）を変える | **組み込みの枠 24 曲**は `src/core/game/bgmPresets.ts`（`PRESET_BGMS`＝slug・曲名・分類 calm/emotion/tense/scene・用途。音の実体は無い・制作プロンプトは `docs/game/bgm-presets.md`）。目録の kind `bgm`（`src/core/game/templates.ts` の `mergeBgmCatalog`＝枠に目録を重ねる・`isBgmReady`＝実体の有無（無ければ「準備中」で選べるが鳴らない・`BGM_PENDING_LABEL` は `template-picker.tsx`）・`catalogBgmKeys`・命名 `bgm-<曲調>-<曲名>.mp3`・ループ区間は `TemplateEntry.loopStart/loopEnd`）。演出は `Cue.bgm`（`preset:bgm/<slug>` か予約キー `BGM_STOP`＝`src/core/game/index.ts`。**次の曲か stop か場面の切れ目（暗転）まで続く**＝`continuity.ts` の bgm レーン・プレイヤーの `bgmAt`・exporter の載せ直し判定の 3 箇所が `sceneBreak` で揃って下ろす）。書き出しは `toNovelGame.ts`（曲が変わるページにだけ `pages[].bgm`・使った曲だけ `bgms[key]`＝zip は `assets/bgm/<slug>.mp3`・投稿は契約 v6 の `asset:tpl-bgm-<slug>`）、プレイヤーは `novelGamePlayer.ts` の `startBgm` `syncBgm`（Web Audio のループ区間・file:// は `<audio loop>` に倒す・メニューの BGM あり/なし）。画面へは `src/ui/game/template-catalog.ts` の `resolveTemplateBgms` `templateBgmKeysOf`（プレビュー・書き出し・投稿の 3 箇所が呼ぶ）、演出エディタの欄は `staging-view.tsx`、試聴は `src/ui/_utils/bgmPlayer.ts`（`toggleCatalogBgm`＝ループ区間ごと・`BgmPreviewButton` は `template-picker.tsx`）。MCP は `set_staging` の `bgm` と `get_staging` の一覧。**プレイヤーとアプリ試聴の 2 箇所を同時に直す** |
| サウンドノベルの grove 公開（契約 v4〜v6）を変える | leaf 側は `src/ui/_api/publish.ts`（`attachEpisodeGames`＝話ごとのプレイヤー同梱・対象は作者が選んだ話だけ＝`novelGameEpisodeOf`・`GAME_FONT_HREF`。**素材の実体は作品ぶん1回**＝`work.gameAssets` と `buildNovelGamePlayer` の `asset:<id>` 参照・契約 v5。音声（BGM・効果音ファイル）を載せるときだけ v6）と公開ページの切り替え（`PublishPage` の話一覧にある行スイッチだけ＝作品ぜんたいの切り替えは無い・`WorkPlatform.novelGameEpisodes`／`novelGame` は前回載せたかの控え）。インライン書き出しは `src/core/exporter/toNovelGame.ts` の `buildNovelGameHtml`。grove 側は novel-platform の `kotonoha-bundle.ts` / `import-work.ts`（契約文書は先方の `novel-platform:docs/architecture/kotonoha-import-contract.md`） |
| 立ち絵（誰をどの席に立たせるか・登録） | **話者とは独立**（D-GAME-SPRITE-FREE）：`Cue.sprites`＝席（l/c/r）ごとの `{ pos, character, expression }`。旧式の `appear`/`expression` は `spriteCuesOf`（`src/core/game/index.ts`）が読み替える。演出エディタ（右の欄の並びは場面の切れ目→背景→切り替え方→BGM→話者→立ち絵→この行から見る→外す・外すのは `ConfirmDialog` 経由・ヘッダに「この話の演出をすべて外す」＝`clearAllCues`）は `src/ui/components/StagingView/staging-view.tsx` の `renderSeatEditor`（左・中央・右の 3 欄＝セリフでも地の文でも同じ。人物を選んでからその席で立ち絵を登録する）。席の選択肢は「変更しない(前の行を引継ぐ)」（既定・空）／「立ち絵なし」（D-GAME-CUE-LABELS）。cue の `hideSprite` はスキーマ・MCP・一覧の印に残るが画面の欄は無い（D-GAME-SPRITE-OFF）。図鑑からは `src/ui/components/GlossaryView/sprite-section.tsx`（人物ページの立ち絵欄）。正本はどちらも `gameAssetRepository.ts`（character/expression で紐づく） |
...
| **X の機能紹介動画**（1 機能＝1 本・字幕つき・日次ルーティンで投稿）の台本・録画・投稿 | 台本は `tools/x-tutorial/scenarios/`（並び＝投稿順は `index.ts`・末尾に足す）、録画と字幕の焼き込み・無料会員として撮る差し替えは `tools/x-tutorial/lib/stage.ts`（Playwright の `page.screencast`）、台本の共通操作は `tools/x-tutorial/lib/app.ts`、X API（OAuth 1.0a・動画アップロード）は `tools/x-tutorial/lib/x-client.ts`。毎日の手順と判断基準は `.claude/skills/x-daily-post/`（アプリ本体とは独立・CI 対象外。README 参照） |
...
| `game/` | サウンドノベル化のドメイン（演出譜＝正本の外・blockId アンカー） | `Staging` `Cue` `SpriteCue`（席ごとの立ち絵・`SPRITE_POSITIONS`・`spriteCuesOf`＝旧式の読み替え） `AssetRef` `MASKED_SPEAKER` `BGM_STOP` / `classifyBlock` `toPages` `applyCues` `findOrphanCues` `suggestSceneBreaks` `suggestSpeaker` / テンプレ背景は `presets.ts`（`PRESET_BACKGROUNDS` `presetBgSvg` `buildGameCredits`・組み込み SVG＝画像が入るまでの控え・`BLACKOUT_BG_KEY`＝予約キー `blackout`「背景なし(ブラックアウト)」`blackoutBgSvg`）／テンプレ立ち絵（シルエット6種）は `spritePresets.ts`（`PRESET_SPRITES` `presetSpriteSvg`）／**運営テンプレの目録**は `templates.ts`（`TemplateManifest` `TemplateEntry` の Zod・`TemplateKind`＝bg/sprite/se/bgm・`mergeBackgroundCatalog` `mergeSpriteCatalog`＝組み込みに目録を重ねる・`mergeBgmCatalog`＝枠 24 曲に目録を重ねる・`isBgmReady`・`catalogBgmKeys`・`parseTemplateFilename` `parseTemplateTsv`・`templateAssetId`＝`tpl-<kind>-<slug>`・`templateUrl`・`applyTemplatePatch`・管理 API の入力 `TemplatePutInputSchema` `TemplatePatchInputSchema`）／持ち込み素材（背景・立ち絵）は `assets.ts`（`UserGameAsset`＝data URL＋tone 3色・キーは `userAssetKey`＝`user:<id>`、テンプレ由来の背景は `gameAssetKey` が `preset` のキーを返す。立ち絵の選定 `pickSprite` `spriteExpressionsOf`、無料枠 `FREE_IMPORT_LIMIT` `importVerdict`、クラウド保管の上限 `HOSTED_ASSET_LIMIT` と判定 `hostedAssetVerdict` もここ）／立ち絵の舞台は `stage.ts`（`resolveStages`＝席 3 つ・話者は明るさだけ・`SPRITE_POSITION_LABELS`）／合成SEレシピは `sePresets.ts`（`PRESET_SES`＝12 種 `presetSe` `seDuration` `sePeriod`・キーは `preset:se/<名>`）／**BGM の枠**は `bgmPresets.ts`（`PRESET_BGMS`＝24 曲・`presetBgm` `presetBgmBySlug` `PRESET_BGM_CATEGORY_LABELS`・実体は持たない）／**機能フラグ**は `features.ts`（`GAME_FEATURES`＝`{ se: false }`。データの形は残したまま画面・MCP・書き出しから外す 1 箇所） |
...
- **運営だけ（遅延ロード）**: `AdminTemplatesPage/`（テンプレ素材の管理：背景・立ち絵・BGM のタブ（効果音は `GAME_FEATURES.se` が立つと出る）。ドロップで一括投入＝ファイル名からキー・画像はブラウザで WebP/サムネ/tone・音声はそのまま長さだけ測る・表示名/分類/時間帯/非表示・BGM のループ区間（秒）・TSV の一括取り込み・音は試聴）
...
| `/api/admin/templates` | 運営テンプレの管理（**staff だけ・それ以外は 404**）。GET=目録 / PUT=`?kind=&slug=` 投入・置き換え（kind bg/sprite は画像 1.5MB＋サムネ、se は mp3/m4a 2MB＋長さ、bgm は mp3/m4a 8MB＋長さ＋ループ区間） / PATCH=表示名・分類・時間帯・並び・非表示・BGM のループ区間・分類の表示名 / DELETE=非表示 |
