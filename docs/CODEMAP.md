...
| 立ち絵の登録・整理（誰に付けるか） | 演出エディタは `src/ui/components/StagingView/staging-view.tsx` の `renderSpriteEditor`（セリフの「話者」欄と地の文の「立ち絵の登場」欄で共用＝**喋らない人物にも付けられる**）。図鑑からは `src/ui/components/GlossaryView/sprite-section.tsx`（人物ページの立ち絵欄）。正本はどちらも `gameAssetRepository.ts`（character/expression で紐づく） |
