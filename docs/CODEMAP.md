...
- **運営だけ（遅延ロード）**: `AdminTemplatesPage/`（テンプレ素材の管理：背景・立ち絵のタブ（効果音は `GAME_FEATURES.se` が立つと出る）。ドロップで一括投入＝ファイル名からキー・画像はブラウザで WebP/サムネ/tone・音声はそのまま長さだけ測る・表示名/分類/時間帯/非表示・TSV の一括取り込み・効果音は試聴）
