...
| サウンドノベル書き出し（ゲーム化・演出譜）を変える | `src/core/game/`（判別・演出譜・テンプレ背景・持ち込み背景・`stage.ts`＝立ち絵の席（左・中央・右）を行ごとに解く `resolveStages`＝書き出しと画面が共用・`continuity.ts`＝「この行で何が効いているか」の解決）+ `src/core/exporter/toNovelGame.ts`（プレイヤー本体は `novelGamePlayer.ts`）。演出エディタは `src/ui/components/StagingView/`（欄の説明は `field-helps.tsx`、アプリ内プレビューは `preview-dialog.tsx`＝`buildNovelGameHtml` を sandbox iframe に流し込む・`startAt` でその行から）、永続化は `src/core/storage/stagingRepository.ts`（演出譜）と `gameAssetRepository.ts`（持ち込み背景） |
...
| `docs/game/bgm-presets.md` | 組み込み BGM 20 曲のファイル名・用途と Suno 用の制作プロンプト（管理ページへ入れる手順つき） |
