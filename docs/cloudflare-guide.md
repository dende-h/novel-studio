...
Cloudflare は「速く配る網」から「エージェントが住み着く実行環境」へ重心を移した。2026年の変更のほとんどは、この一文で説明がつく。Pages より Workers、ブラウザ描画より Browser Run、AutoRAG より AI Search、`McpAgent` より `createMcpHandler`。名前が変わったものはどれも、人間のアクセスを前提にした設計を、機械のアクセスを前提にした設計で置き換えている。
...
- **読み方**: 1〜3章は初回に通読してほしい。全体地図・エージェントの実体・MCP の現在地で、4章以降を読むための足場になる。4章（WebMCP）は例外で、実装を判断するつもりなら通読、しないなら結論の 4-6 だけでよい。5章以降は引く用で、下の目次から必要な章だけ開いて構わない。9章に逆引き表、10章にこのリポジトリでの優先順位、末尾に用語集と結びを置いた。
...
5. 推論と検索: Workers AI / AI Gateway / AI Search
6. エージェントに手足を与える: ブラウザ・サンドボックス・ファイルシステム
7. 土台側で起きたこと（既存アプリに効く変更）
8. 迎える側の話: 来るエージェントに値段を付け、通し方を決める
9. 逆引き: 「〜したい」から引く
...
2026年に増えたエージェント向けの機能は、**すべて Durable Objects の応用として理解できる**。新しい実行基盤が来たわけではない。この一点を押さえておくと、後続の章がだいぶ軽くなる。以下はその根拠にあたる見取り図で、プロダクトを下の階と上の階に分けて考えると迷いにくい。

下の階は昔からある「世界中に置いた拠点でリクエストを受ける仕組み」——CDN、DNS、WAF、Bot 管理。ここは基本的に変わらない。コトノハで言えば、独自ドメインを載せて TLS を張り、Bot を弾いている部分がこれにあたる。

上の階が「その拠点でアプリのコードを走らせる仕組み」で、2020年代に増築が続いてきた。JavaScript を実行する Workers があり、その Worker ごとに1個の状態と SQLite を持たせる Durable Objects が乗る。データは SQL の D1 と、オブジェクトストレージの R2 に置く。コトノハはこの階に建っている。

2026年に増えたのは、この上の階に新しく設けられた区画——エージェントを走らせる場所だ。Agents SDK、Browser Run、Sandbox、Dynamic Workers、AI Gateway、AI Search。これらはどれも既存の Workers と Durable Objects の上に乗る。冒頭の一文はこの構造を指している。

#### 付表1: 名前が変わったもの、地位が変わったもの
...
Pages については、コトノハが現に Pages Functions で動いているので、10章で移行の要否を別に扱う。この階の上に乗るエージェントの実体を、次章で開く。
...
| Durable Objects SQLite | Free は読み 500万行/日、書き 10万行/日、5GB | Paid は月 250億行の読みと 5,000万行の書き、5GB-月まで込み。超過は読み $0.001/百万行、書き $1.00/百万行、$0.20/GB-月 |
...
D1 は2026-09-01 から、無料プランで日次の行読み書き上限を超えるとクエリが失敗するようになった。それまでは超過しても通っていたので、無料枠で動かしている環境は注意がいる。

### 2. エージェントの正体は「眠るサーバ」— Agents SDK

一般に AI エージェントというと、モデルにツールを渡してループを回すプログラムを指す。そのループの中身を、Agents SDK は引き受けない。引き受けるのは**ループを何日も生かしておく側**だ。ここを取り違えると、SDK の API がなぜこの形なのか分からなくなる。
...
Durable Object は次の3つで退去（eviction）させられる。ここに出てくる秒数と頻度が SDK の設計理由そのものなので、覚えておく価値がある。

- リクエストも WebSocket もない状態が 約70〜140秒続いたとき
- コード更新やランタイム再起動のとき（1日1〜2回、タイミングは非決定的）
- alarm ハンドラが15分を超えたとき
...
#### 2-3. チャットを作るなら `AIChatAgent` から始めればいい

継承の階層は `DurableObject` → `Server` → `Agent` → `AIChatAgent` の順で、`DurableObject` に近いほど自由、`AIChatAgent` に近いほど出来合いになる。さらにその外側に、ループそのものを引き受けるハーネスがある。実際に選ぶのは次の4つのどれかになる。
...
見出しのとおり、チャットなら `AIChatAgent` から始めるのが素直だ。メッセージは SQLite に自動で保存され、通信が切れてもストリームの途中から再開でき、複数タブへ WebSocket でブロードキャストされる。行サイズが SQLite の上限に近づいたら自動で圧縮もする。
...
効果の実測値は、Cloudflare 自身の API MCP サーバが出している。API 全体を素の MCP ツールとして並べると 2,594 ツール・約117万トークン、必須パラメータだけに削っても約24万トークン。Code Mode なら `search()` と `execute()` の2ツール、約1,000トークンで済む。
...
`DynamicWorkerExecutor` は既定で `fetch()` と `connect()` を塞ぐ（`globalOutbound: null`）。モデルが外部APIを直に叩くコードを書くとここで失敗するので、外に出る必要があるツールは、サンドボックスの外——ホスト側のツール実装——に置く。`console.log` は捕捉されて実行結果に入り、タイムアウトは既定30秒。`wrangler` 側には `[[worker_loaders]] binding = "LOADER"` が要る。

#### 2-6. 記憶の管理には別の層が用意されている

会話が長くなったときの圧縮と、会話をまたぐ長期記憶は、状態管理とは別の API になっている。

Session API（`agents/experimental/memory/session`）は、システムプロンプトを「コンテキストブロック」に分けて組み立て、古いメッセージを要約で畳む。元のメッセージは SQLite に残り、要約は読み出し時に被せる非破壊のオーバーレイとして働く。トークン数の見積もりには tiktoken を使わず、`max(文字数/4, 単語数*1.3)` の概算で済ませている。tiktoken の 80〜120MB のヒープが Workers の 128MB 制限に収まらないための割り切りだと、ドキュメントに明記されている。
...
なお、エージェント名にリクエストIDやユーザーIDを混ぜてはいけない。ダッシュボード上で無数の別エージェントに見えてしまう。渡す識別子は3つあり、役割が違う。ダッシュボードでエージェントを束ねる単位が「名前」（`booking-agent`）、その中で本番と検証を分けるのが「ID」（`booking-agent-production`）、1本の会話を追うのが会話IDである。

### 3. MCP サーバは「建てる」から「セッションを持たない関数」へ変わった

前章のエージェントは、自分でツールを持つか、外から借りてくる。借り方の標準がここで扱う規格になる。外部のAIクライアントに自分のアプリの操作を渡す仕組みを、Model Context Protocol（MCP）と呼ぶ。ツール（呼べる関数）、リソース（読めるデータ）、プロンプト（定型の指示）の3つを JSON-RPC 2.0 で公開する。コトノハが `/api/mcp` で提供しているものがまさにこれで、Claude や ChatGPT から作品を編集できるのはこの規格のおかげである。

2026年のMCPまわりの変更は、ひとことで言えば**セッションを捨てた**ことに尽きる。

#### 3-1. 2026-07-28 仕様: リクエストごとに使い捨てのサーバ

新しい仕様（2026-07-28）では、リクエスト1本ごとに新しいサーバを組み立てて捨てる。MCP プロトコル上のセッションも、それを保持する Durable Object も要らない。managed のリモートMCPサーバは2026-07-28にこの方式へ切り替わり、Agents SDK は v0.20.0 で対応した。

旧クライアント（2025年の Streamable HTTP）からのステートレスなリクエストも同じ `/mcp` が受ける。設定変更なしに繋ぎ直せるクライアントがほとんどだ。ただしSSE トランスポートを強制する設定のクライアントだけは動かない。`/sse` という URL は残っているが、中身は Streamable HTTP のエイリアスに置き換わっていて、旧 HTTP+SSE の実装はもう応答しない。クライアント側を Streamable HTTP か自動判別に変える必要がある。
...
そして `McpAgent` は**非推奨かつ機能凍結**である。既存のサーバは移行対象で、新規に書いてはいけない。ドキュメントにも「新しい `McpAgent` サーバを作らないこと」と明記された。機能凍結とは、これから MCP 仕様に入る機能がこちらの経路には来ないという意味で、いま書けば仕様が進むたびに置き去りになる。
...
状態を持ちたい場合はどうするか。カウンタを覚える、ゲームの盤面を保持する、前回のAPI呼び出しの結果をキャッシュする——こうした用途は `McpAgent` の存在理由だった。いまは Agent 側に状態を持たせ、MCP ハンドラからその Agent を呼ぶ形に分ける。

#### 3-3. 認可は OAuth 2.1 のサブセット。4つの型がある

MCP の認可は OAuth 2.1 のサブセットで規定されている。`workers-oauth-provider` というライブラリが用意されていて、次の4通りで使える。

- Cloudflare Access を認可サーバにする
- GitHub や Google など第三者の OAuth プロバイダに繋ぐ
- Stytch・Auth0・WorkOS のような認可サービスを挟む
- Worker 自身がフロー全体を実装する

第三者プロバイダを使う場合も、MCPクライアントに渡すトークンは MCP サーバ側——つまり自分の Worker——が発行する。上流のトークンをそのまま横流しすると、GitHub なり Google なりの権限がそのままクライアントの手に渡り、こちらから失効させる手段がなくなる。
...
ステートレス版では、ツールから `context.http.authInfo` を読めば標準のトークンメタデータが取れる（`McpAgent` の `this.props` に相当する）。
...
#### 3-5. 自分のアカウントを AI に触らせるなら managed のサーバがある
...
管理者はポータルごとに公開するツールとプロンプトを選べる。名前と説明を上書きするエイリアスも張れるので、上流のサーバに手を入れずに「AIが選び間違えにくい名前」に直せる。文脈量を削るオプションもある。`optimize_context=minimize_tools` はツールの説明とスキーマを剥がして名前だけにし、必要になった時点で `query` ツールで取りに行かせる。これで最大5倍のトークン節約になる。ポータル自体に Code Mode を効かせることもでき、ポリシーは off / opt-in / on by default / enforced の4段階。
...
ここから先は、必要になった章だけ開けばよい。4章がブラウザ側の新しい規格、5〜7章が製品ごとの現在地、8章が自分のサイトに来るエージェントの扱い、9章が症状からの逆引き、10章がこのリポジトリでの優先順位である。
...
なぜこれが要るのか。今のエージェントによるブラウザ操作は、スクリーンショットを撮る→どこを押すか考える→クリックする、のループでできている。遅いうえに、UIが少し変わるだけで壊れる。WebMCP なら `searchFlights()` や `bookTicket()` を型付きの引数で直接呼べる。changelog はこの違いを一文で説明している。「インターネットは人間のために作られたので、AIエージェントとして回遊するのは今のところ信頼性が低い」。
...
両者は競合しない。同じページで、ページ固有の操作（スクロール、フォーム入力）を WebMCP に、サーバ側の操作（保存、決済）をサーバMCPに割り振る。エージェントからは1つの道具箱に見える——という組み合わせが想定されている。

#### 4-2. ブラウザAPIの現状: 置き場所が `navigator` から `document` へ動いた

【要確認】この節のブラウザ側の記述は、Cloudflare のドキュメント外の情報にもとづく。Chrome のバージョン番号、Origin Trial の期間、API の移動時期については、公式仕様と Chrome のドキュメントに直接あたって裏を取れていない。**実装する前に一次情報を確認してほしい。**

そのうえで、把握している範囲を書く。仕様は W3C の Web Machine Learning Community Group が策定していて、最初の提案が 2026年2月10日に公表された。Chrome では 149 から Origin Trial が走っている。

ここで注意がいるのは、**APIの置き場所が仕様の途中で移動したこと**だ。初期の Chrome ビルドは `navigator.modelContext` に生やしていた。その後、ツールはページに属するという整理から、仕様側が `document.modelContext` へ移した。Chrome 150 で `navigator.modelContext` は非推奨になり、アクセスすると初回に警告が出る別名として残る。初期の `provideContext()` / `clearContext()` も2026年3月の改訂で仕様から削除され、いまは `registerTool()` / `unregisterTool()` だけが宣言手段になっている。

ドキュメントも Agents SDK の実装も、いまは `navigator.modelContext` を前提に書かれている。両方に登録しておくのが安全だと筆者は考える。
...
#### 4-3. 呼ぶ側 — Browser Run が WebMCP 対応サイトを操作する

Cloudflare の WebMCP 対応は、呼ぶ側と名乗る側の両方にある。別の製品で、名前が似ているので混同しやすい。この節が呼ぶ側、次の 4-4 が名乗る側にあたる。
...
#### 4-4. 名乗る側 — Agents SDK のブリッジがサーバのツールをページに登録する

`agents/experimental/webmcp` は逆向きで、自分のMCPサーバのツールを、ページ内のブラウザAIから呼べるようにするアダプタだ。名前が紛らわしいので整理すると、これは「サーバのツールをブラウザの道具箱に相乗りさせる橋」である。
...
**`watch` は SSE が要る。** サーバが 405 を返す場合、アダプタは自動追随なしで動き続ける。ツールの増減はページを開き直すまで反映されない。

**`timeoutMs` は1呼び出しごと。** 全体の上限ではないので、多数のツールを連続で呼ぶ処理には別の制御が要る。

**ブラウザ専用。** Worker 内やサーバサイドレンダリング中には import できない。動くには `navigator.modelContext` を持つブラウザが要る。

アダプタも下地のブラウザAPIも実験段階で、リリース間で変わりうると明記されている。Playwright によるテスト（探索・実行・破棄・並行実行・ページネーション・watch 更新・エラー系）は `cloudflare/agents` リポジトリの `packages/agents/src/webmcp-tests/` に揃っているので、挙動の確認はそちらが早い。

#### 4-6. コトノハに当てはめるとどうなるか
...
エージェントは推論を回し、外の知識を引く。その2つを Cloudflare で賄うと3点セットになり、重ねて使う前提で設計されている。Workers AI が推論の実行、AI Gateway が全モデルの入口、AI Search が検索付きの回答生成である。

#### 5-1. モデルを回す — Workers AI

料金は Neuron という単位で、$0.011 / 1,000 Neurons。無料枠は1日10,000 Neurons で、Free と Paid の両方に付く（Paid は超過分が課金されるだけ）。リセットは毎日 00:00 UTC。

2026-07-28 から、重いモデルの一部が課金手段を要求するようになった。当初は `@cf/moonshotai/kimi-k2.6`、`@cf/moonshotai/kimi-k2.7-code`、`@cf/zai-org/glm-5.2` の3つで、その後 `@cf/zai-org/glm-5.3` 系と `@cf/deepseek-ai/deepseek-v4` 系が料金ページの対象に加わった。Workers Paid プランか、AI Gateway のプリペイド残高のどちらかがあれば使える。無料プランのまま叩くと 403（内部エラー 5035）が返る。無料のまま使えるモデルには `@cf/zai-org/glm-4.7-flash`、`@cf/google/gemma-4-26b-a4b-it`、`@cf/nvidia/nemotron-3-120b-a12b` がある。
...
課金の統合（Unified Billing）を使うと、Workers AI と OpenAI・Anthropic など外部プロバイダの支払いが1つのクレジット残高にまとまる。副次的な効果が2つある。プリペイド残高があれば Workers Paid プランなしでも上記のフロンティアモデルを叩ける。そしてその3モデル（kimi-k2.6・kimi-k2.7-code・glm-5.2）のレート制限が、アカウント単位かつモデル単位で毎分20リクエストから50リクエストに上がる。どちらもエージェント用途では効いてくる差だと思う。

外部モデルは `openai/gpt-5.5` のような `provider/model` 形式で、Cloudflare の認証だけで REST から叩ける。

#### 5-3. 自前の文書に答えさせる — AI Search
...
`context` という名前のカスタムメタデータを付けておくと、その値が各チャンクに添えられて回答生成時にモデルへ渡る。文書の要約や出典URLを入れておく使い方が案内されている。メタデータは1ベクトルあたり合計10KiB の枠に収まる必要があり、フィルタに使えるのは各文字列の先頭64バイトまで。カスタムフィールドは1インスタンスあたり5個まで。

この章でコトノハに効きそうなのは日本語の埋め込みモデルだけで、AI Search は暗号化の設計と衝突する。詳しくは10章で扱う。
...
#### 6-1. ウェブページを見る・操作する — Browser Run
...
Quick Actions を使うには `compatibility_date` を `2026-03-24` 以降にする必要があり、ローカルの `wrangler dev` では browser バインディングに `remote: true` が要る。

#### 6-2. Worker Loader が製品になり、課金が付いた — Dynamic Workers

実行時にコードを読み込んで、隔離された Worker として動かす仕組み。バインディングを絞る、Tail Worker でログを取る、`fetch()` を塞ぐ、リソース上限を掛ける、といった制御を呼び出し側が全部握れる。

Durable Object Facets を使うと、動的に読み込んだコードを自前の SQLite を持つ Durable Object として動かせる。テナントごとに違うコードが動く SaaS を作るときの部品になる。
...
#### 6-3. Linux が要る処理を隔離して走らせる — Sandbox SDK と Containers

Sandbox SDK は Containers の上に立つ薄いAPIで、`getSandbox(env.Sandbox, "user-123")` から `exec()` でコマンドを打つ。サンドボックスごとに専用のVMが立つ。

GA までに入った機能は、プレビューURL、Python/JS/TS の永続的なコードインタープリタ、ブラウザから使える PTY 端末、ワークスペースのバックアップと復元、ファイル変更の監視と幅広い。ここまで揃うと、エージェントのコーディングセッションを止めて再開する運用が組める。

セキュリティ面で効くのが Outbound Workers による認証情報の注入だ。送信ハンドラは Workers ランタイム側（サンドボックスの外）で動くので、サンドボックス内のコードに秘密を見せずに、外へ出る直前でトークンを付けられる。TLS の中身の検査、許可・拒否リスト、インスタンスごとの動的なポリシーも書ける。信頼できないコードを走らせる前提の設計として筋がいい。

Sandbox SDK 1.0 は `@next` タグでプレビュー中。新規は `@next` を勧める案内が出ている。

#### 6-4. 作業場と保管場所 — `@cloudflare/computer` と Artifacts
...
この章の道具はどれもコトノハの現状には過剰で、必要になるとすれば Code Mode を入れるときの Dynamic Workers だけである。

### 7. 土台側で起きたこと（既存アプリに効く変更）

エージェントの話をいったん離れる。土台側の変更を先に潰しておくと、あとの判断が軽くなる。

#### 7-1. 無料枠が本当に止まるようになった

これまで無料プランの上限は目安に近く、超えても通っていた。2026年に入ってそれが変わった。

D1 では 2026-09-01 以降、Workers Free プランで日次の行読み・行書き上限を超えるとクエリがエラーを返す。上限に達するとメールで警告が飛び、UTC の深夜にリセットされる。保存済みデータには影響しない。

Durable Objects の SQLite ストレージも 2026-01-07 から課金対象になった。読み・書き・保存容量の3軸で、単価は D1 と揃えてある。数え方に癖があって、`setAlarm()` は1回につき1行の書き込み、KV 形式のメソッド（`get` `put` `delete` `list`）も内部の隠しテーブルへの読み書きとして課金される。見返りとして30日ぶんの Point-in-Time Recovery が付く。

#### 7-2. `compatibility_date` を上げると挙動が変わるもの

`deleteAll()` が alarm も消すようになった（`compatibility_date` が `2026-02-24` 以降）。これまでは `deleteAlarm()` を別に呼ぶ必要があり、忘れると alarm のメタデータが残ってストレージが解放されなかった。

#### 7-3. 長い処理と、外との受け渡しが楽になった

**Workflows の上限が上がった。** Workers Paid なら1インスタンスあたりのステップ数が既定10,000、設定で25,000まで。ただし Workers Free は従来どおり1,024のままなので、無料プランで動かしているワークフローは分割の必要が残る。`step.sleep` はこの上限に数えない。

再試行まわりも細かくなった。`step.do()` の第2引数の `ctx.attempt` で現在が何回目の再試行かを読める。リトライ間隔には関数を渡せるので、「レート制限エラーなら長く待ち、ネットワークエラーなら短く」といった分岐が書ける。課金は 2026-08-10 以降に開始（公式表記は「2026年8月10日より早くは課金しない」）で、Free プランはステップとストレージの課金対象外である。
...
Python と JavaScript の Worker が RPC で相互に呼べるようにもなった。Service バインディング経由で、追加の依存もスキーマ定義もシリアライズのコードも要らない。例外は呼び出し元へ伝播する。

R2 SQL には分析系の構文が入った。ウィンドウ関数、`QUALIFY`、`DISTINCT ON`、集合演算（`UNION` / `INTERSECT` / `EXCEPT`）、`GROUPING SETS` / `ROLLUP` / `CUBE`、`MEDIAN` や `PERCENTILE_CONT` などの厳密な集計。R2 Data Catalog（R2 バケットに組み込みの Apache Iceberg カタログ）に入れたテーブルを `wrangler r2 sql query` で引ける。

### 8. 迎える側の話: 来るエージェントに値段を付け、通し方を決める

ここまでは作る側だった。自分のサイトに来るエージェントをどう扱うかも、2026年に道具が増えた領域である。コトノハには今のところ直接効かないが、公開サイトを持つ以上、選択肢は知っておいて損がない。

#### 8-1. AIクローラーを通す・止める・課金する

AI Crawl Control の pay per crawl は、AIクローラーに対してゾーン単位で価格を設定する仕組みだ。支払い意思のないアクセスには `HTTP 402 Payment Required` と価格を返す。決済は Cloudflare が Merchant of Record として持つ。クローラー側は Web Bot Auth で身元を証明し、`crawler-exact-price` か `crawler-max-price` を署名対象に含めて送る（ヘッダの改竄と再送攻撃を塞ぐため、この署名は必須になった）。最低価格は1クロールあたり $0.001 で、クローラーごとに「課金する / 無料で通す / 遮断する」を選べる。private beta、参加は申請制。

ここに落とし穴がある。検索エンジンのクローラーを遮断・課金すると、そのまま検索結果からの流入が落ちる。ダッシュボードの Category 列で検索エンジンを区別してから設定する。

もう1つ、Redirects for AI Training は `<link rel="canonical">` を既に張っているサイトなら、トグル1つで有効にできる。検証済みのAI学習クローラーが重複ページや廃止ページを取りに来たとき、canonical 先へ 301 を返す。人間・検索エンジン・AI検索エージェントには元のページを返す。Pro 以上で追加費用なし。

#### 8-2. エージェント同士が払い合う仕組み

HTTP 402 を土台にした支払いプロトコルが2つ、Agents SDK に組み込まれている。x402（Coinbase 発、Cloudflare が x402 Foundation の創設メンバー）はステーブルコイン決済で、`PAYMENT-REQUIRED` `PAYMENT-SIGNATURE` `PAYMENT-RESPONSE` の3ヘッダを使う。Machine Payments Protocol（MPP）は `WWW-Authenticate: Payment` を使い、カード（Stripe 経由）や継続課金にも対応し、x402 とも後方互換がある。サーバ側は `withX402` と `paidTool`、クライアント側は `withX402Client` を使う。これで `402` の処理と人間の承認を挟める。

#### 8-3. 組織の中でAIがどう使われているかが見える

Gateway が MCP 通信を自動判別するようになり、Insights & Logs に AI security report が追加された。DLP は ChatGPT・Claude・Gemini・Perplexity へのプロンプトを検査してトピック分類できる。社内でのAI利用を把握したい場合の入口になる。

### 9. 逆引き: 「〜したい」から引く

症状から引く索引。矢印の先の数字は節番号で、「→ 2-3」なら2章の3節を指す。
...
エージェントの中で完結し、途中経過を自分で管理するなら fiber。ステップごとの再試行と数日単位の待機が要るなら Workflows。→ 2-2、7-3

#### 信頼できないコードを走らせたい → Dynamic Workers か Sandbox SDK
JavaScript が数十ミリ秒で動けばいい場合は Dynamic Workers。Linux のバイナリやパッケージマネージャが要るなら Sandbox。→ 6-2、6-3

#### ログイン後の画面をエージェントに操作させたい → Browser Run + Live View
セッションを一時停止して人間がログインし、同じタブのまま再開できる。→ 6-1

#### 自分のドキュメントについて答えさせたい → AI Search
R2 に文書を置いて `aiSearch()` を呼ぶ。テナント分離はフォルダ + `folder` フィルタ。→ 5-3

#### AIクローラーからの収益化・遮断 → AI Crawl Control
価格設定は private beta、遮断は今すぐできる。→ 8-2
...
コトノハのMCPサーバは `2025-06-18` をプロトコル版として宣言し、クライアントが要求した版をそのまま返している。JSON-RPC 2.0 の Streamable HTTP で実装しているので、Cloudflare 側の `/sse` 廃止の影響は受けない。ただし接続してくるクライアントが SSE トランスポートを強制していないかは確認する価値がある。もし旧 HTTP+SSE を前提にしたクライアントが残っていれば、Streamable HTTP か自動判別に切り替えてもらう案内が要る。

2026-07-28 仕様への対応そのものは急がなくていい。新仕様のサーバは、旧クライアントが送るステートレスなリクエストもそのまま受ける。既存の実装がすぐ壊れる話ではない。ただし自前実装である以上、仕様の追随はこちらの仕事になる。Agents SDK の `createMcpHandler` へ寄せれば互換の維持を任せられるが、いまの実装は OAuth 中継を含めて自前で組み上がっているので、載せ替えのコストは小さくない。判断は別途。

#### 影響が中くらい: Pages → Workers の移行を検討対象に載せる

新規プロジェクトを Workers に寄せる方針は明言されていて、Pages のドキュメントにも警告が出るようになった。ただし**Pages が止まるとは書かれていない**。静的アセットは Pages でも Workers でも無料で、Pages Functions は Workers と同じ単価で課金されるので、コスト面の差はない。

移行して得られるのは機能差だ。

- Durable Objects を同じプロジェクト内で持てる。いまは別 Worker に切り出してバインディングを張る必要がある
- Cron Triggers が使える
- Observability が充実している

逆に手間になるのはこの3つ。

- `pages_build_output_dir` を `assets.directory` に置き換える設定変更
- `functions/` ディレクトリを Worker にコンパイルする作業
- プレビュー環境の作り分けの設計し直し。いまの `[[env.preview.d1_databases]]` による本番/stg 分離をどう再現するか

エージェント機能を本気で足すなら Durable Objects が要るので、そのときが移行の潮時になる。逆に言えば、いまのまま同期とバックアップを回すだけなら急ぐ理由はない。

#### 影響が小さい: 日本語埋め込みモデルと AI Search

将来、作品横断の検索や「似た話」の抽出をやるなら、`@cf/pfnet/plamo-embedding-1b`（日本語特化）と `@cf/baai/bge-m3`（多言語・100言語以上）が候補になる。R2 に本文を置く構成なら AI Search がそのまま乗る。ただしコトノハはローカルファーストで本文が IndexedDB にあり、クラウド側には暗号化ブロブしか置かない設計なので、そのままでは AI Search に食わせられない。検索機能をサーバ側に置くかどうかは、暗号化の設計まで戻って考える必要がある。

#### 様子見: WebMCP

4-6 に書いたとおり、ブラウザAPIの置き場所が移動中で、戻り値の平坦化もコトノハの構造化データと相性が悪い。仕様が Chrome の安定版に入るまで待つ。
...
冒頭に書いたとおり、2026年の変更は「人間のアクセスを前提にした設計を、機械のアクセスを前提にした設計で置き換える」という一本の線でつながっている。コトノハもその線の上にいて、すでに MCP サーバを1本公開している以上、無関係ではいられない。ただし全部を今やる必要はない。時間軸で3つに分かれる。

**今すぐ確認する。** コトノハの `/api/mcp` に接続してくるクライアントが、SSE トランスポートを強制していないか。本家の `/sse` は Streamable HTTP のエイリアスに置き換わり、旧 HTTP+SSE は応答しなくなった。自前実装なので Cloudflare の変更に直接壊されはしないが、クライアント側の前提がずれていれば繋がらない。

**条件が揃ったら動かす。** Pages から Workers への移行は、Durable Objects が必要になった時が潮時になる。エージェント機能を1つでも本気で足すなら Durable Objects は避けて通れず、そのとき Pages のままでいる理由がなくなる。逆に、いまのように同期とバックアップを回すだけなら急ぐ理由はない。

**待つ。** WebMCP は、ブラウザ側の API が `navigator` から `document` へ移動している最中で、戻り値も文字列に潰れる。Chrome の安定版に入り、構造化された戻り値が扱えるようになってから再検討すればよい。

順番を間違えなければ、この線に乗るのは難しくない。危ないのは、動いているものを新しさだけで置き換えることのほうだ。
...
- **この文書で解決しない疑問は**: novel-studio リポジトリの Issue に投げてほしい。回答がそのまま次の改訂の材料になる。自分で調べるなら、Cloudflare のドキュメントは `https://developers.cloudflare.com/<product>/llms.txt` に製品ごとの索引がある。AI に調べさせるなら `https://docs.mcp.cloudflare.com/mcp` を MCP サーバとして繋ぐのが速い。調べた結果でこの文書を直すところまでやってもらえると、いちばん助かる。
- **改訂の方針**: 質問が集中した章だけを厚くする。質問の出ない章はそのままでよい。全章を均等に更新し続ける必要はない。
