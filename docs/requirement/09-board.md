...
| **D-BOARD-PENNAME** | **ペンネームはアカウント 1 つにつき 1 つ**にする。以前は同じ言葉で 3 つの値が動いていた——端末ローカルの `Profile.penName`（サイドバー・新しい作品の著者）、Clerk のフルネーム／メール（ヘッダに出ていた「サインアップで入れた名前」）、`board_profiles.display_name`（掲示板）。ローカルの値は端末に貼り付いているので**アカウントを切り替えても変わらず**、ヘッダには名乗るつもりのない本名やメールが出ていた。**サーバの表示名を正本**とし、ローカルはその写しにする（端末に「誰の名前か」の印を**別のキー**で置き〈`profile-account`〉、サインインのたびに `penNameForAccount`（`src/core/profile/account.ts`）で突き合わせる。**`Profile` 本体には足さない**——あれは端末間で同期され〈`profile:me`・LWW〉クラウドバックアップにも入るので、欄が増えると canonical JSON のハッシュがずれ、まだ更新していない端末が知らないキーを落として押し返す〈Zod は未知のキーを捨てる〉＝押し合いになる）。読む側（ヘッダ・サイドバー・`createWork` の著者既定）は今までどおり `Profile.penName` を読むだけでよい。**勝手にサーバへ登録はしない**＝表示名は全体で一意（`name_key` の UNIQUE）なので、黙って登録すると同じ名前の別の人が先着で弾かれる。登録・改名は**プロフィールのダイアログ 1 か所だけ**（`ProfileDialog`・Root が 1 つ持ち、ヘッダの名前とサイドバーのプロフィール欄がどちらもそこを開く）。掲示板の `NameDialog` は「まだ決めていない人」への入口として残す。**コトノハ-grove- （grove）の作者名とは、あえて揃えない**（D-PENNAME-GROVE）。 |
| **D-PENNAME-GROVE** | **コトノハ-leaf- のペンネームと コトノハ-grove- の作者名は、別々に持つ。** 揃えることは技術的にはできる（`POST /api/authors/register` は upsert で、呼び直しが改名として通る）が、揃えない。コトノハ-grove- の `profiles.display_name` は**読者ニックネームと作者ペンネームを兼ねる 1 フィールド**で、作品一覧・検索・公開通知・いいね通知がすべてこれを読む。コトノハ側の保存に追随させると、(1) 公開済みの全作品の著者名がその場でまとめて変わり、(2) 読者としてのニックネーム（コメント・いいねに出る名前）まで変わり、(3) `register` が要求する `agreedGuidelines: true` を本人の操作なしに再送することになる。**代わりに、混乱の芽を文言で摘む。** ①コトノハ-grove- へ送るバンドルの `author` は先方が無視し、登録済みの作者名を常に優先する（platform の `import-work.ts`）ので、投稿画面に `AuthorNameCard`（「作者名」）を出して**押す前に名前を見せる**。確認ダイアログにも名前を入れる。②**語を分ける**——コトノハ側は「ペンネーム」、コトノハ-grove- 側は「作者名」。同じ語を当てると「片方を変えたらもう片方も変わる」と読まれる（作者登録カードの欄も「ペンネーム」から「作者名」へ改名した）。③`ProfileDialog` は「コトノハ-grove- の作者名は別の設定なので、ここでは変わりません」と言い切る。④コトノハ-grove- の作者名は**プロフィール 1 つが全作品に及ぶ**（作品に焼き付かない）ので、改名しても「同じ人なのに作品ごとに名前が違う」は起きない。その事実も投稿画面に書く。 |
