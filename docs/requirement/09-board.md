...
| **D-BOARD-OPEN** | スレ立ては**最初から全員に開放**。空の一覧を見せないよう、運営が呼び水スレを先に立てておく（§2）。摩擦を足す代わりに、レート制限（スレ 10 本/日・投稿 10 件/時）で守る。**スレの日次上限が守るのは一覧の読みやすさで、サーバの資源ではない**（1 スレは D1 の 2 行。外向きの取得は OGP だけで、そちらは投稿単位の枠が縛る）。一人が並べると他の人の話が下へ押し出される、という一点のために置いている。当初 3 本にしていたが、ふつうに使っていて当たる数だったうえ、**運営自身が呼び水スレで引っかかった**ので 10 本へ緩め、**staff は数えない**ことにした（`functions/api/board/threads.ts`）。外しはしない——スレ立ては投稿枠も食う（本文が `board_posts` の 1 件）ので日次を外しても 10 件/時＝1 日 240 本までは立てられ、その気になった一人で一覧が埋まる。 |
...
| **D-BOARD-RATE** | レート制限は**二段構え**にする。(1) 設計の上限（投稿 10 件/時・スレ 10 本/日。スレの日次は staff を数えない）は D1 の行を数えて判定する（`countPostsSince` / `countThreadsSince`。削除済みも数える＝消して書き直す抜け道を作らない）。(2) 連打と自動化を止める**分あたりの安全弁**は既存 `checkRateLimit`（`functions/api/_lib/rate-limit.ts`）を流用し、**キーは `board:${userId}`** にする。あの表は `user_id` が主キーの 1 行で同期の 60 req/min を数えているので、素で渡すと掲示板の投稿が同期の枠を食う。スキーマ変更は要らない。**`checkRateLimit` の窓は 60 秒なので、そこへ「10 件/時」の値を渡してはいけない**（10 件/分＝600 件/時になり、設計の 60 倍緩む）。安全弁は掲示板の全操作が同じ 1 行を共有するため、操作ごとに違う値を渡さず一律 60/分にする（👍 だけ 10 にすると、一覧で 10 回押した人がその 1 分間まったく書けなくなる）。**この数値を変えるときは `public/board-guidelines.html` も直す**。ガイドラインに「スレッドは1日に10本まで、書き込みは1時間に10件まで」と数字を書いて公表しているので、実装値だけ動かすと案内が嘘になる。 |
