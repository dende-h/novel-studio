...
| **D-BOARD-NOTICE** | **`notice`（お知らせ）は運営だけが立てられる。** スレ立ての可否は `canCreateThread`（`src/core/board/permission.ts`・`STAFF_ONLY_KINDS`）が決め、member の notice は 403。**返信は誰でもできる**（返信の可否は今までどおり `canPost`）＝お知らせを読んだ人がその場で聞き返せる。👍 と運営ステータスは付けない（`KINDS_WITH_STATUS` に入れない。運営からの連絡に「検討中」を付けても意味がない）。一覧では塗りつぶしのチップと行の色（`KIND_UI.notice.rowClassName`）で目立たせる。スレを立てる画面には**選択肢そのものを立場で出し分ける**（`creatableKindOrder(role)`）＝押させてから 403 で断らない。 |
