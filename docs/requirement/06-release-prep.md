...
### A-2. Cloudflare D1（メタデータ）
- [ ] `wrangler d1 create novel-studio` → 出力の `database_id` を `wrangler.toml` に記入（コミット）。
- [ ] `pnpm d1:migrate:remote` で本番 D1 にマイグレーション適用（`0001_init` セッション ＋ `0002_sync_works`）。
      ⚠️ **このエージェント環境では auto-mode がリモート D1 操作を拒否する**ため、リモート適用は
      **人間が手元のターミナルで実行**（セッション内なら `! pnpm d1:migrate:remote`）。
- [ ] 適用確認：`wrangler d1 execute novel-studio --remote --command "SELECT name FROM sqlite_master WHERE type='table'"`
      で `sessions` / `works` / `rate_limits` が存在すること。

### A-3. Cloudflare R2（同期の本文・画像ブロブ）
...
- [ ] `pnpm exec wrangler r2 bucket create novel-studio-media`（binding `MEDIA` は `wrangler.toml` に配線済み）。
      ⚠️ **バケット未作成のまま deploy すると Functions が起動時に失敗する**
      （`R2 bucket 'novel-studio-media' not found`）。
      ※ wrangler はプロジェクトの devDependency。素の `wrangler` は PATH に無いので
      **必ずプロジェクト直下で `pnpm exec wrangler ...`**（または `npx wrangler`）で呼ぶ。

### A-4. `ENCRYPTION_KEY`（at-rest 暗号化鍵）
- [ ] `openssl rand -base64 32` で **base64 32byte ちょうど**の鍵を生成（不正長だと起動時 throw）。
- [ ] `wrangler pages secret put ENCRYPTION_KEY`（**stg / 本番の各スコープ**）。ローカルは `.dev.vars`。
- [ ] 生成した鍵を**安全に保管**。
      ⚠️ **鍵ローテーション不可（現状）**：差し替えると既存 R2 ブロブが復号不能になり、
