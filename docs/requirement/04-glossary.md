...
>
> **【2026-08 更新】** 本書が「辞書 →（P1.1 で）図鑑」と呼んでいた器は、現在の名称が **「用語集」** で、
> 作者だけの決め事は別の器（プロットの**世界観設定**）へ分けた。以下の本文は当時の設計判断の記録として
> そのまま残す（名称は当時のもの）。現行の器の分け方・改名の理由は
> [08-worldbuilding.md](./08-worldbuilding.md) を見ること。
