...
import { BOARD_LIMITS, VoteInputSchema } from '../../../src/core/board/types'
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (!pollRow) return json({ error: 'no_poll' }, 404)
  ...
  if (!votable.ok) return json({ error: votable.reason }, 409)

  // 範囲外・重複・単一選択なのに複数、を弾いて昇順に正規化する。
  const choices = normalizeChoices(parsed.data.choices, poll)
  if (!choices.ok) return json({ error: 'bad_choices', reason: choices.reason }, 400)

  // 流量の安全弁（D-BOARD-RATE）。**キーは `board:` 接頭辞**＝rate_limits は user_id 1 行の
  // 表なので、素の userId を渡すと同期の 60 req/min の枠を掲示板が食う（§7-11）。
  ...
  if (!written.ok) return json({ error: 'already_voted' }, 409)
