  ...
  return (
    <FieldHelp title="話者" description="このセリフを誰が言ったか。画面下の名前枠に出ます。">
      ...
      <Item label="名前を出したくないとき">
        ？？？を選ぶと名前枠が ？？？ になり、立ち絵も出ません。空（名前を出さない）にすると、
        名前枠そのものが消えます。
      </Item>
    </FieldHelp>
  )
...
export function SpriteHelp() {
  return (
    <FieldHelp title="立ち絵" description="話者に登録した絵から、この行で使う表情を選びます。">
      <Item label="（自動：通常）とは">
        その人に最初に登録した1枚です。表情を足すと、ここで選べるようになります。
      </Item>
...
export function AppearHelp() {
  return (
    ...
    >
      ...
      <Item label="すでに立っている人には効きません">
        表情の切り替えはセリフ側（話者と表情）の担当です。
      </Item>
    </FieldHelp>
  )
}
...
export function BgHelp() {
  return (
    <FieldHelp title="背景" description="この行から背景が変わります。">
      <Item label="（変えない）のとき">
        前の背景のまま進みます。背景は一度変えると、次に変えるまで続きます。
      </Item>
...
export function SeHelp() {
  return (
    <FieldHelp title="効果音" description="この行が表示された瞬間に、1回鳴ります。">
      <Item label="音のファイルは増えません">
        端末でその場で合成する音なので、書き出しても中身は重くなりません。
      </Item>
      <Item label="読者は切れます">
        ゲーム側のメニューで「効果音：なし」にできます。音が出せない場所でも読めるようにするためです。
      </Item>
      <Item label="試聴">横の「試聴」で、ここで鳴る音をそのまま確かめられます。</Item>
    </FieldHelp>
  )
}
