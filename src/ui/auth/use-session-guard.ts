 ...
 * 別端末に奪われたら onSuperseded() を呼ぶ（呼び出し側がこの端末を強制ログアウト＝ゲスト化する）。
 */
export function useSessionGuard(onSuperseded: () => void): { claimed: boolean } {
  ...
  // onSuperseded も同様に ref へ退避（参照変化で監視 effect を作り直さない）。
