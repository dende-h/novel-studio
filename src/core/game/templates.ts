 ...
 * （bgmPresets.ts の 24 曲。音の実体は持たない＝同じ名前の曲が目録に入るまで「準備中」）。
 *
...
/**
 * BGM の曲調の語 → 表示名（組み込み 24 曲の 4 分類＋07-novel-game.md §4.3 の旧 5 種）。
 * 目録の表示名があればそちら。
 */
...
export interface CatalogBgm {
  ...
  entry?: TemplateEntry
  /** 組み込みの枠（bgmPresets.ts の 24 曲）。目録だけの曲には無い */
  builtin?: PresetBgm
  hidden: boolean
  durationMs?: number
  loopStart?: number
...
/**
 * BGM の一覧＝組み込みの枠 24 曲（bgmPresets.ts）に目録の曲を重ね、目録だけにある曲を後ろに足す。
